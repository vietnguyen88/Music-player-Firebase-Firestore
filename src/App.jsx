...
function App() {
  ...
  return (
    <>
      <div className="vinyl ">
        <img src={vinyl} alt="" className={`${playerStatus ? "active-spin": " "}`} />
      </div>
      {tracks && (
        <div className="audio">
          <p>{tracks[0]?.name}</p>
          <audio controls src={tracks[0]?.path} type="audio/mpeg" onPlay={()=> setPlayerStatus(true)} onPause={()=> setPlayerStatus(false)}></audio>
        </div>
      )}
      <Upload/>
